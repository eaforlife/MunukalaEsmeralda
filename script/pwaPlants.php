...
$pwaquery = "SELECT *, hexel_plant.plant_id AS plantID FROM hexel_plant;";
$pwaSQL = $conn->query($pwaquery);
if($pwaSQL->num_rows > 0) {
	while($row = $pwaSQL->fetch_assoc()) {
		?>
		<div class="card">
			<div class="card-header" id="plant-<?php echo $row['plantID']; ?>">
				<h2 class="mb-0">
					<button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse" data-target="#collapse-<?php echo $row['plantID']; ?>" aria-expanded="true" aria-controls="collapse-<?php echo $row['plantID']; ?>">
						...
						<span class="text-capitalize"><?php echo $row['plant_name']; ?></span>
					</button>
				</h2>
			</div>
			
			<div id="collapse-<?php echo $row['plantID']; ?>" class="collapse" aria-labelledby="<?php echo $row['plantID']; ?>" data-parent="#plantAccordion">
				<div class="card-body">
					<img src="./style/img/<?php echo $row['plant_image']; ?>" class="img-fluid" alt="<?php echo $row['plant_name']; ?>">
					<?php
						$plantDescQuery = "SELECT * FROM hexel_plant_desc WHERE desc_plantid = '" . $row['plantID'] . "' ORDER BY desc_published DESC LIMIT 1;";
						$plantDescSQL = $conn->query($plantDescQuery);
						if($plantDescSQL->num_rows > 0) {
							while($plantDescRow = $plantDescSQL->fetch_assoc()) {
					?>
					...
					<p class="m-0 p-0"><?php echo html_entity_decode($plantDescRow['desc_plant']); ?></p>
					...
					<p class="m-0 p-0"><?php echo html_entity_decode($plantDescRow['desc_guide']); ?></p>
					...
					<p class="m-0 p-0">PHP <?php echo $plantDescRow['desc_price_min']; ?> - <?php echo $plantDescRow['desc_price_max']; ?></p>
					<p class="m-0 p-0 text-muted"><small><?php if($plantDescRow['desc_price_comment'] != 'NULL') { echo $plantDescRow['desc_price_comment']; } ?></small></p>
					<?php
							}
						}
					?>
					...
					<?php
						$galleryQuery = "SELECT * FROM hexel_plant_gallery WHERE plant_id = " . $row['plantID'] . ";";
						$gallerySQL = $conn->query($galleryQuery);
						if($gallerySQL->num_rows > 0) {
							while($galleryRow = $gallerySQL->fetch_assoc()) {
							?>
								<div class="container">
									<div class="row">
								<?php echo "<div class='col-sm-4'><img src='./style/img/" . $galleryRow['image_name'] . "' loading='lazy' id='gallery-" . $galleryRow['gallery_id'] . "' width='300'></div>"; ?>
									</div>
								</div>
							<?php
							}
							...
							echo "<p class='text-muted m-0 p-0'>Gallery seems to be empty.</p>";
						}
					?>
				</div>
			</div>
	...
	echo "<h3 class='text-muted'>Plant database seems to be empty.</h3>";
}
$pwaSQL->close();
